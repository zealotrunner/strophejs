    ...
    mixins: (function(){
        var DiscoItems = Strophe.Mixin.apply({
            ...
            getFeatures: function() {
                var featureElems = this.getElementsByTagName('feature');
                var discoFeatures = [];

                // shove the items in an array
                if (featureElems != null && featureElems.length > 0){
                    for (var i=0; i < featureElems.length; i++){
                        var feature = featureElems[i];

                        var featureVar = feature.getAttribute('var') || null;

                        feature = Strophe.Mixin.apply(feature, {
                            featureName: featureVar,
                        });
                        discoFeatures.push(feature);
                    }
                }
